...
import type { Bindings, ExposeOptions } from '../core/types.js';
...
export function useExpose(id: string, bindings: Bindings, options: ExposeOptions = {}): void {
  ...
  useEffect(() => {
    ...
    const proxyBindings = stableBindingsRef.current;
    if (!proxyBindings) return;
    const unregister = registry.register(id, proxyBindings, {
